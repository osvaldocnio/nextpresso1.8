...
sub level_2{
	...
	if(-d $downsamplingDir){
		...
		if($doDownsampling){
			...
			my $command="perl ".$Bin."preprocess.pl --nThreads ".$maximunNumberOfInstancesAllowedToRunSimultaneouslyInOneParticularStep." --mode 2 --seqtkPath ".$seqtkPath;
			$command.=" --conditions ".$conditions." --samples ".$ALLsamples." > ".$workspace."downsampling.log 2>&1";
			print $logfh (Miscellaneous->getCurrentDateAndTime()).$command."\n";

			#tophat has to wait for dowsampling
...
sub level_6{
    	my($comparisons,$deseqParams,$extraPathsRequired,$htseqcountPath,$htseqcountParams,$samtoolsPath,$samples,$GTF,$maximunNumberOfInstancesAllowedToRunSimultaneouslyInOneParticularStep,
