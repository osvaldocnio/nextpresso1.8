...
sub main(){
	...
	my $seqtkPath=${$configHashRef}{seqtkPath}[0];
	...
	my $executionCreatedTempDir="/tmp/".$experimentName;
	...
	if($level=~ /2/){
		...
		$queueSystem,$queueName,$multicore,$queueProject);
	...
	if($level=~ /6/){
		if($doSpikesAndGenomeRefIndexing eq "false"){		
			ExecutionLevels::level_6($comparisons,$deseqParams,$extraPathsRequired,$htseqCountPath,$htseqcountParams,$samtoolsPath,$samples,$GTF,$maximunNumberOfInstancesAllowedToRunSimultaneouslyInOneParticularStep,$workspace,$experimentName,$logfh,
			$executionCreatedTempDir,$queueSystem,$queueName,$multicore,$queueProject);
...
sub checkProgramPaths($){
	...
	if(${$configHashRef}{seqtkPath}[0]!~ /\/$/){
		${$configHashRef}{seqtkPath}[0]=${$configHashRef}{seqtkPath}[0].="/";
	}
	$program=${$configHashRef}{seqtkPath}[0]."seqtk";
 	if (!-e $program){
		print STDERR "\n[ERROR]: The program ".$program." doesn't exist\n\n";
		exit(-1);
	}
