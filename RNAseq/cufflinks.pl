...
#	} 
	...
	if($mode=~ "1"){		
		...
		calculateCorrelationsAndPCA_GeneLevel($experimentName,$outDir,$samples);
		calculateCorrelationsAndPCA_IsoformLevel($experimentName,$outDir,$samples);
	}
	if($mode=~ "2"){
...
sub calculateCorrelationsAndPCA_GeneLevel($$$){
	...
	&cufflinks::calculateCorrelationsAndPCA_GeneLevel($experimentName,$outDir,$samples);
}

sub calculateCorrelationsAndPCA_IsoformLevel($$$){
	my ($experimentName,$outDir,$samples)=@_;
	
	&cufflinks::calculateCorrelationsAndPCA_IsoformLevel($experimentName,$outDir,$samples);
}
